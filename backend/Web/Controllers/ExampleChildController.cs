...
namespace Web.Controllers
{
  ...
  public class ExampleChildController : ApiControllerBase
  {
    ...
    [HttpGet]
    public async Task<ActionResult<List<BaseExampleChildQueryDto>>> GetAllChildren(CancellationToken cancellationToken)
    {
      return await Mediator.Send(new GetExampleChildrenQuery(), cancellationToken);
    }

    [HttpGet("{id}")]
    public async Task<ActionResult<ExampleChildDetailsQueryDto>> GetChildDetails([FromRoute] int id, CancellationToken cancellationToken)
    {
      return await Mediator.Send(new GetExampleChildDetailsQuery
      {
        Id = id
      }, cancellationToken);
    }
